...
A sample of $100$ first bags, all $840$ prefixes each, gave nine failures in three
bags --- a rate of $1.1 \times 10^{-4}$ over the sample --- and a fourth bag,
$\mathrm{TJLISZO}$, had been settled earlier with four. Those thirteen were all we had
when the structural results below were found; the exhaustive sweep has since
superseded them, and we quote it instead.

Two structural facts cut the work. The first is that the hold slot is empty only on an
initial segment of the play: a move either places the arrival and leaves the hold
alone, or places the held piece and puts the arrival in its place, or --- only from an
empty hold --- stashes the arrival without placing anything. The last two leave the
hold occupied, so a play stashes at most once.
...
order. Since a play stashes at most once, it either never stashes, giving
...
bags at once and permutes the $840$ prefixes among themselves, so a first bag and its
mirror agree prefix by prefix. Reflection commutes with the exchange, and neither
fixes any first bag --- the exchange because the first two pieces of a bag are
distinct, the mirror because J and L each occur once --- so together they cut the
$5040$ orders to $1260$ classes of four, which is the form in which the exhaustive
computation is worth attempting.

The description of plays can be sharpened, and the sharper form says how far
Theorem~\ref{thm:swap} extends. Stash the first arrival at once --- by the theorem
this costs nothing --- and call it $a_0$, with $a_1 a_2 \ldots a_m$ arriving after.
Exactly one arrived piece is pending at any time, so a play is precisely a choice of
the steps $j_1 < \cdots < j_r$ at which the held piece is placed: step $j_k$ receives
$a_{j_{k-1}}$ (with $j_0 = 0$), every other step receives its own arrival, and
$a_{j_r}$ is left in the hold. One piece at a time may be deferred, and releasing it
defers whatever stood in its place. In this form Theorem~\ref{thm:swap} is the
bijection $C \leftrightarrow C \bigtriangleup \{1\}$ between chains.

That bijection is special to the front.
...
\begin{proof}
Let $B$ be the board reached before the two steps, $x$ the held piece and $a, b$ the
two arrivals. Two steps later the reachable states are $(Ba, x)$ and $(Bx, a)$ with
$b$ pending, which the exchange fixes, together with $(Bab, x)$, which it sends to
$(Bba, x)$; and placing $a$ before $b$ reaches boards that placing $b$ before $a$ does
not. At the front there is no held piece $x$, which is why the branches coincide there
and nowhere else. For the verdict, at $w = 10$ and $n = 10$ the first bag
$\mathrm{JTLIOSZ}$ with prefix $\mathrm{LOZS}$ admits no perfect clear while
$\mathrm{JLTIOSZ}$, its exchange of the second and third arrivals, does.
\end{proof}

The counterexamples are scarce: over the $250{,}316$ decided pairs available to us the
exchange preserved the verdict $250{,}293$ times and failed $23$. A rule that holds in
$99.99\%$ of a quarter of a million cases is still not a theorem. Equality of search
cost is no evidence either, and fails in both directions: exchanging $q_0$ and $q_1$
agreed on the state count to the digit across $1{,}684$ solvable cases but differed on
the first failing pair ($38{,}542{,}538$ against $38{,}537{,}388$), while the exchange
of Proposition~\ref{prop:adjacent}, which need not preserve the verdict, often
preserves the count exactly --- two of its four branches coincide.

One caution about the computation changed an answer for us. A first pass with a state
cap of $8 \times 10^6$ reported \emph{no} failures at all: each was sitting in a case
that hit the cap, undecided. A capped search is not a negative result.
...
At the other end, no first bag with the I first has been made to fail: the $72$ late
classes of that kind are settled over all $840$ prefixes, and the other $288$ were
tested against the six prefixes that have produced failures, all $1728$ cases possible
and none undecided. The most expensive settled in $7.9 \times 10^6$ states, an order
of magnitude below the $3.7$--$5.6 \times 10^7$ the known failures demand. This is a
probe, not a proof: six of $840$ prefixes is under one percent.

A proof of Theorem~\ref{thm:family} by counting is not merely missing but impossible,
and the obstruction disqualifies every tool in this paper.
...
Of the regularities we proposed for the failing prefixes, one survives: across all
$94$ failures known to us the prefix is a permutation of $\{O,S,Z\}$ together with one
of $J$ or $L$, without exception, which is the shape the mirror lemma forces. Three
others broke, each having held for every case then available --- that the prefixes end
in $\mathrm{ZS}$, that they end in $S$ or $Z$ (the endings now stand at $S$ fifty-nine
times, $Z$ thirty and $O$ five), and that they take only three shapes.

The most instructive was the lateness condition. Every failing first bag then known
was late; of a hundred sampled bags, fifteen were late and only three of those failed;
and a prediction recorded before the last undecided cases were settled --- that they
would succeed, none being late --- came out right. It survived until the $180$ late
classes were done and the sweep moved on, where $\mathrm{JTZOLSI}\,\mathrm{OJZS}$ and
$\mathrm{OSTLJZI}\,\mathrm{OJSZ}$ fail with $Z$ third and $S$ second. The prediction
had in fact tested something else: all four of those cases had the I first, which the
table above shows to be the easy end. A confirmed prediction need not confirm the
reason it was made for.

The mechanism one would guess for lateness remains natural --- a vertical I is the main
way to resolve an S/Z staircase inside four rows, and an I arriving after the staircase
is built cannot undo it, the direction Burgiel's theorem~\cite{burgiel1997} points in;
see also~\cite{brzustowski1992}. The computation says the guess does not survive
contact with the position of the I.
