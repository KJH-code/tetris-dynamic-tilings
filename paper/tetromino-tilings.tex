...
% TODO: fill in the sampled failure rate once the bag1 sweep completes.
% Confirmed so far: two exhaustive first bags (IOTSZJL: 0 failures out of 840;
% TJLISZO: 4 failures out of 840, namely OJZS, OLZS, LOZS, JOZS).

Two first bags have been settled exhaustively. For $\mathrm{IOTSZJL}$ all $840$
prefixes admit a perfect clear. For $\mathrm{TJLISZO}$ exactly four do not:
$\mathrm{OJZS}$, $\mathrm{OLZS}$, $\mathrm{LOZS}$ and $\mathrm{JOZS}$. All four end
in $\mathrm{ZS}$, and the two near misses that do succeed end in $\mathrm{OZ}$; the
first bag itself ends in $\mathrm{S},\mathrm{Z},\mathrm{O}$. The mechanism appears to
be that vertical I is the main way to resolve an S/Z staircase within four rows, and
that a late I cannot repair what the earlier pieces have built --- the direction
Burgiel's theorem~\cite{burgiel1997} points in.

We stress the limits of this. Two first bags out of $5040$ is not a basis for a
failure rate, and our search is hard-drop only, so a reported impossibility is an
impossibility \emph{in our model}: spins and tucks might rescue some of these cases.
The community's own perfect-clear solvers handle a more general model and can make
the same observations, so we claim no novelty here; the point is only that perfect
clears are almost always available, and that the well-known openers exist to save
...
$260{,}423$ perfect clears at widths $4$, $5$, $6$ and $8$, by two independent
