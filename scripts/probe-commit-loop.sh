...
#
# 환경변수 SRC: 탐침이 **작업디렉터리**에 쓰고 이 루프가 주기마다 레포로 복사한다.
# 탐침이 레포 파일에 직접 추가하면 작업 트리가 몇 초마다 더러워져서, 커밋을 해도
# 곧 다시 더러워진다 (스톱 훅이 매 턴 걸린다). SRC 를 쓰면 **레포 파일은 커밋 시점에만
# 바뀐다.** 대가는 마지막 커밋 이후의 진행분을 작업디렉터리와 함께 잃을 수 있다는 것뿐이고,
# 주기가 10 분이면 최대 10 분이다.
set -u
...
cd "$REPO"
while true; do
  if [ -n "${SRC:-}" ] && [ -f "$SRC" ]; then cp "$SRC" "${files[0]}"; fi
