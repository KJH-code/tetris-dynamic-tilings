...
\author{Jaehyeok Kim\\[2pt]
\small Seoul Science High School, Seoul, Republic of Korea}
\date{\today}
...
tetromino types --- I, T, J and L --- admit a perfect clear at every width $w \geq 4$,
and O admits one at every even width; we determine the minimum number of pieces in
each case, for all widths. The two exceptions are S and Z, which admit no perfect
clear at any width, alone or together. The classification extends to every subset of
the seven types.
...
Pak~\cite{kornpak2004} later analysed the structure of those tilings, and
Reid~\cite{reid2005} placed questions of this kind in the general setting of
Klarner systems.
...
T & Walkup: $4 \mid w$ and $4 \mid n$ & every width $w \geq 4$, $n = w$ \\
J, L & & every width $w \geq 4$, $n = w/2$, $w$ or $2w$ \\
S, Z & gap number conjectured unbounded~\cite{hochberg2015} & \textbf{never, at any width} \\
\bottomrule
\end{tabular}
\end{center}
...
type~\cite{mithardness2026}; Baccherini and Merlini~\cite{baccherini2008} study the
combinatorics of related games. Our question is different in kind --- not the complexity
...
$(0,3,0)$ and $(0,0,1)$. For the first four, $N_1 \geq 2h_0 + 3p_0 \geq 6$. The
...
\begin{theorem}
\label{thm:sets}
Let $w \geq 4$. A set $P$ of tetromino types admits a perfect clear at width $w$ if
and only if $P$ meets $\{I, T, J, L\}$, or $w$ is even and $O \in P$.
\end{theorem}

\begin{proof}
If $P$ contains one of I, T, J, L, use that type alone
(Theorems~\ref{thm:I},~\ref{thm:T},~\ref{thm:JL}); each of those constructions is
available at every $w \geq 4$. Otherwise $P \subseteq \{O,S,Z\}$, and the profiles of
all three lie in $\{(1,2,1),(2,2)\}$, so the counting derivation in the proof of
Theorem~\ref{thm:S} applies to their total counts without change: horizontal pieces
are unused, every piece starts at an even column, and at odd widths $R = 0$. At even
widths, if $O \in P$ then $w/2$ copies of O clear two rows; if not, then $P \subseteq
\{S,Z\}$ and Theorems~\ref{thm:S} and~\ref{thm:SZ} apply.
\end{proof}

\begin{remark}[Widths below four]
The hypothesis $w \geq 4$ is needed, and T is the reason. I clears every width from
$1$ upwards and J and L clear widths $2$ and $3$, so those constructions lose nothing;
but T alone clears neither width $2$ nor width $3$, although the counting system
permits $R = 4$ at both. (Width $1$ admits no T placement at all, since every rotation
of T spans at least two columns.) The classification at $w \leq 3$ would therefore
need $T$ removed from the list, and we have not determined which mixed sets containing
T succeed there.
\end{remark}
...
Width $16$ continues the pattern and is the most expensive case we have settled: the
unrestricted minimum is $4$, and the seven-bag minimum is $12$. Both $n = 4$ and
$n = 8$ are impossible --- every admissible multiset was decided --- while $n = 12$
admits five, among them one copy each of I and L with two of every other type.
...
One structural fact halves the work for anyone repeating this.

\begin{lemma}
\label{lem:hold}
The hold slot is empty on an initial segment of the play and never afterwards, so a
play stashes at most once.
\end{lemma}

\begin{proof}
A move either places the arriving piece and leaves the hold unchanged, or places the
held piece and puts the arriving one in its place, or --- only from an empty hold ---
stashes the arriving piece without placing anything. The last two leave the hold
occupied, and a stash requires it empty.
\end{proof}

Write $\mathrm{HG}_m(h; s_1 s_2 \ldots)$ for the set of length-$m$ sequences the player
can place starting from hold $h$ with $s_1, s_2, \ldots$ arriving, each step placing
either the held piece (the arrival replacing it) or the arrival (the hold unchanged).

\begin{theorem}
\label{thm:swap}
If the arrival sequence $q_0 q_1 \ldots q_{L-1}$ has $L \geq n+1$, then the set of
placement sequences of length $n$ the player can achieve is
\[
q_1 \cdot \mathrm{HG}_{n-1}(q_0; q_2 q_3 \ldots) \;\cup\; q_0 \cdot \mathrm{HG}_{n-1}(q_1; q_2 q_3 \ldots),
\]
which is symmetric in $q_0$ and $q_1$. Exchanging the first two arrivals therefore
changes neither the achievable placement sequences nor whether a perfect clear exists.
\end{theorem}

\begin{proof}
The two terms are the two continuations after stashing $q_0$ immediately, so they are
achievable. For the converse, note that inside $\mathrm{HG}$ the move ``place the held
piece, keep the arrival'' repeated from hold $q_1$ emits $q_1, q_2, q_3, \ldots$ in
order. By Lemma~\ref{lem:hold} a play either never stashes, giving
$q_0 q_1 \ldots q_{n-1}$, which is $q_0$ followed by $n-1$ such repetitions and needs
the arrival $q_n$ --- the one place where $L \geq n+1$ is used --- or stashes once, at
$q_k$ say, giving $q_0 \ldots q_{k-1}$ followed by $\mathrm{HG}_{n-k}(q_k; q_{k+1}
\ldots)$; for $k = 0$ this is the displayed union, for $k = 1$ its second term, and for
$k \geq 2$ it is reached inside that second term by $k-1$ repetitions before branching
freely.
\end{proof}

So the $5040$ first bags fall into $2520$ classes of two, and a bad first bag always has
a bad partner. The hypothesis is not decorative: with $L = n$ no stash is possible and
the play is forced, and among sequences of four pieces at width $4$ there are $66$ pairs
differing only in their first two entries where one clears and the other does not.

The mirror of Lemma~\ref{lem:mirror} acts here too. Reflecting a play reflects both
bags at once, and since we run every one of the $840$ prefixes for each first bag, the
prefixes are permuted among themselves; so a first bag and its mirror image give the
same verdict prefix by prefix. Reflection commutes with the exchange of
Theorem~\ref{thm:swap}, and neither fixes any first bag --- the exchange because the
first two pieces of a bag are distinct, the mirror because J and L both occur exactly
once --- so the two together partition the $5040$ orders into $1260$ classes of four,
which is the form in which the exhaustive computation is worth attempting.

Lemma~\ref{lem:hold} can be sharpened into a complete description of the plays, which
is what tells us how far Theorem~\ref{thm:swap} extends.

\begin{lemma}[Chain form]
\label{lem:chain}
Write the arrivals as $a_0$ (stashed at once, which costs nothing) followed by the
stream $a_1 a_2 \ldots a_m$. A play of length $m$ is exactly a choice of positions
$j_1 < j_2 < \cdots < j_r$ in $\{1, \ldots, m\}$ --- the steps at which the held piece
is placed. Position $j_k$ receives $a_{j_{k-1}}$, with $j_0 = 0$; every position
outside the chain receives its own arrival; and $a_{j_r}$ is what remains in the hold.
\end{lemma}

\begin{proof}
At each step the player places either the held piece or the arrival, so exactly one
arrived piece is pending at any time. Placing the held piece at step $j$ makes $a_j$
pending, which is the passage from $j_k$ to $j_{k+1}$; placing the arrival leaves the
pending piece unchanged. Reading the chain off a play and building a play from a chain
are mutually inverse.
\end{proof}

So one piece at a time may be deferred, and releasing it defers whatever stood in its
place. Theorem~\ref{thm:swap} now has a second proof: exchanging $a_0$ with $a_1$
matches the chains $C$ and $C \setminus \{1\}$ or $C \cup \{1\}$ bijectively.

That bijection is special to the front, and the reason is worth stating because it
bounds what one may hope for.

\begin{proposition}
\label{prop:adjacent}
Exchanging two adjacent arrivals $q_i, q_{i+1}$ with $i \geq 1$ does not in general
preserve the achievable placement sequences, nor even the existence of a perfect clear.
\end{proposition}

\begin{proof}
Let $B$ be the board reached before the two steps and $x$ the held piece. Writing
$a, b$ for the two arrivals, the states available two steps later are
\[
\{(Ba, x),\ (Bx, a) \ \text{with } b \text{ pending}\} \cup \{(Bab, x)\},
\]
against $\{(Bb, x),\ (Bx, b)\} \cup \{(Bba, x)\}$ after the exchange. The two branches
that defer one of the pieces coincide; the branch that places both differs, and
placing $a$ before $b$ reaches boards that placing $b$ before $a$ does not. At the
front the held piece $x$ does not yet exist, which is why the branches there coincide
completely. For the verdict, at $w = 10$ and $n = 10$ the first bag $\mathrm{JTLIOSZ}$
with prefix $\mathrm{LOZS}$ admits no perfect clear while the exchange of its second
and third arrivals, $\mathrm{JLTIOSZ}$, does.
\end{proof}

The gap between the two statements of Proposition~\ref{prop:adjacent} is wide in
practice: over the $250{,}316$ decided pairs available to us the exchange preserved the
verdict $250{,}293$ times and failed $23$ times. A rule that holds in $99.99\%$ of a
quarter of a million cases is still not a theorem.

Equality of search cost is likewise no evidence of equivalence, and the two phenomena
are independent in both directions. Exchanging $q_0$ and $q_1$ leaves the
stash-immediately subtree identical from the second step onwards, so a search that
succeeds inside it visits the same states for both orders; across $1{,}684$ solvable
cases the two orders agreed on the state count to the digit. Yet on the first failing
pair we tested --- where the no-stash branch, which is not identical, must be explored
--- the verdicts agreed as the theorem requires while the counts differed,
$38{,}542{,}538$ against $38{,}537{,}388$. In the other direction the exchange of the
second and third arrivals, which Proposition~\ref{prop:adjacent} shows need not
preserve the verdict, frequently preserves the count exactly: two of its four branches
coincide, and when the search succeeds inside those the totals agree to the digit.
...
\subsection*{What the exhaustive computation shows so far}

The sweep over the $1260$ classes is running. We report the parts of it that are
complete, together with counts of the failures found so far; the latter are as of the
$305$ classes settled at the time of writing and can only grow. Say that a first bag
is \emph{late} if $S$ and $Z$ both arrive in position five or later. Exactly $180$ of the $1260$ classes are late, and all of them are now
settled over all $840$ prefixes, with nothing left undecided: of those $151{,}200$
cases, $92$ admit no perfect clear, and they fall in $31$ of the $180$ classes.
Arranged by the position of the I piece, the failing classes are

\begin{center}
\begin{tabular}{lrrrrrr}
\toprule
position of I & 1 & 3 & 4 & 5 & 6 & 7 \\
\midrule
late classes & 72 & 36 & 36 & 12 & 12 & 12 \\
of which fail & 0 & 4 & 12 & 12 & 2 & 1 \\
\bottomrule
\end{tabular}
\end{center}

There is no column for position $2$: by Theorem~\ref{thm:swap} the first two arrivals
are interchangeable, so no class has its I there. Every late class with the I first
succeeds, every late class with the I fifth fails, and the difficulty falls away again
at positions six and seven. The effect is therefore not monotone in the position of
the I, and the naive reading of the table --- that moving the I earlier cannot hurt ---
is false. Moving it from sixth to fifth turns a possible position into an impossible
one in $68$ of the $72$ matched pairs, and in none of the $72$ does the earlier I help.

One family is completely decided and is the sharpest impossibility we have.

\begin{theorem}[computer-verified]
\label{thm:family}
Let the first bag consist of $T, J, L, O$ in any of the $24$ orders, followed by
$I, S, Z$, and let the second-bag prefix be one of $\mathrm{JOZS}$, $\mathrm{JZOS}$,
$\mathrm{OJZS}$. Then no perfect clear exists, in all $72$ cases. Mirroring gives the
same statement for the tail $I, Z, S$ with the prefixes $\mathrm{LOSZ}$,
$\mathrm{LSOZ}$, $\mathrm{OLSZ}$.
\end{theorem}

At the other end, no first bag with the I first has been made to fail. All $72$ late
classes of that kind are settled over all $840$ prefixes, and the remaining $288$
classes with the I first were tested against the six prefixes that have produced
failures, $1728$ cases, all possible and none left undecided. The most expensive of
those searches settled in $7.9 \times 10^6$ states, an order of magnitude below the
$3.7$--$5.6 \times 10^7$ that the known failures demand. We stress that this is a probe
and not a proof: six of $840$ prefixes is under one percent of the untested classes.

A proof of Theorem~\ref{thm:family} by counting is not merely missing but impossible,
and the obstruction is worth recording because it disqualifies every tool in this
paper.

\begin{proposition}
\label{prop:nostatic}
Whether a perfect clear exists is not a function of the multiset of arriving pieces.
Consequently no invariant computed from the multiset of placed pieces --- column
counting as in Lemma~\ref{lem:count}, the parity identity of
Appendix~\ref{app:parity}, or a Walkup-style argument --- can decide the question.
\end{proposition}

\begin{proof}
At $w = 10$, $n = 10$, the arrival sequences $\mathrm{TJILSZO}\,\mathrm{OJZS}$ and
$\mathrm{TJLISZO}\,\mathrm{OJZS}$ differ by one transposition of adjacent letters and
so present the same eleven pieces, hence the same candidate multisets of ten placed
pieces. The first admits a perfect clear and the second does not.
\end{proof}

Nor does the position of the I determine the verdict on its own, once one leaves the
family of Theorem~\ref{thm:family}: $\mathrm{JTLIOSZ}$ fails on $\mathrm{LOZS}$ while
$\mathrm{JLTIOSZ}$ succeeds on it, and both have their I fourth.

\subsection*{Patterns proposed and refuted}

Of the regularities we have proposed for the failing prefixes, one survives and three
do not. Across all $94$ failures known to us the prefix is a permutation of
$\{O,S,Z\}$ together with one of $J$ or $L$, without exception; this is the shape the
mirror lemma forces, and it is the only one that has held.

The others were each supported by every case then available. That the failing prefixes
end in $\mathrm{ZS}$ broke at thirteen failures; that they end in $S$ or $Z$ broke at
twenty-nine, on $\mathrm{LZSO}$ after $\mathrm{JOTILSZ}$, and the endings now stand at
$S$ fifty-nine times, $Z$ thirty and $O$ five. That the failing prefixes take
only three shapes broke the same day.

The most instructive was the lateness condition. All four failing first bags then known
were late, fifteen of a hundred sampled bags were late and only those three failed, and
a prediction recorded before the last four undecided cases were settled --- that they
would succeed, since none was late --- came out right. It survived until all $180$ late
classes were settled and the sweep moved on to the rest, where
$\mathrm{JTZOLSI}\,\mathrm{OJZS}$ and $\mathrm{OSTLJZI}\,\mathrm{OJSZ}$ fail with $Z$
third and $S$ second respectively. The prediction had in fact tested something else: all
four of those undecided cases had the I first, which the table above shows to be the
easy end.

The mechanism one would guess for the lateness condition is still the natural one ---
a vertical I is the main way to resolve an S/Z staircase inside four rows, and an I
arriving after the staircase has been built cannot undo it, the direction Burgiel's
theorem~\cite{burgiel1997} points in; see also~\cite{brzustowski1992}. What the
computation says is that the guess does not survive contact with the position of the I.
...
$4{,}758{,}805$ perfect clears at widths $4$, $5$, $6$, $7$ and $8$, by two independent
implementations agreeing on the number of plays in every case.
...
% TODO before submission: given names not yet confirmed (surnames only below).
...
% TODO before submission: page range unconfirmed, 97--105 or 94--102 depending on source.
