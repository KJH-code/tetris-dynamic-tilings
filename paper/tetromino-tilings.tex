...
Theorem~\ref{thm:swap} also explains a stronger effect we first met as a coincidence.
Exchanging $q_0$ and $q_1$ leaves the stash-immediately subtree identical state by
state, so a search that stashes first and succeeds inside that subtree visits exactly
the same states for both orders; across $1{,}684$ solvable cases the two orders agreed
on the state count to the digit. The no-stash branch is \emph{not} identical --- an
I placed before an O reaches boards an O placed before an I cannot --- and it is
explored only when the search fails. On the first failing pair we tested, the two orders
agreed on the verdict, as the theorem requires, and differed in state count,
$38{,}542{,}538$ against $38{,}537{,}388$.
...
Across all thirteen failures reported above the second-bag prefix is a permutation of
...
direction Burgiel's theorem~\cite{burgiel1997} points in; see
also~\cite{brzustowski1992}.
