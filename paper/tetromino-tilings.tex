...
One structural observation cuts the work in half for anyone repeating this. The hold
slot starts empty, so the first two arrivals can be played in either order, and swapping
them appears to leave the problem unchanged: in every instance we have compared --- two
pairs of first bags over all $840$ prefixes each, and a third pair over four prefixes,
$1{,}684$ cases in all --- the two orders agree not only on the verdict but on the exact
number of states the search visits. The $5040$ first bags would then fall into $2520$
classes of two, halving the cost of the exhaustive computation. We have not proved this.
